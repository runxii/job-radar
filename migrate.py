...
def migrate():
    ...
    matched.columns = matched.columns.str.strip()  # Remove any leading/trailing whitespace from column names
    # Read unmatched sheet
    unmatched = pd.read_excel(EXCEL_PATH, sheet_name="unmatched", dtype={"id": str})
    unmatched.columns = unmatched.columns.str.strip()  # Remove any leading/trailing whitespace from column names
